...
#include "macro.cuh"
#include "stdio.h"
//__device__ void output_type_cl_increment(output_type_cl* x, const change_cl* c, float factor, float epsilon_fl);
//__device__ inline float norm3(const float* a);
__device__ inline void normalize_angle(float* x);
//__device__ inline bool quaternion_is_normalized(float* q);
//__device__ inline void angle_to_quaternion(float* q, const float* rotation, float epsilon_fl);
//__device__ inline void angle_to_quaternion_multi(float* qa, const float* qb);
//__device__ inline void quaternion_normalize_approx(float* q, float epsilon_fl);
__device__ void quaternion_increment(float* q, const float* rotation, float epsilon_fl);
//__device__ inline  float vec_distance_sqr(float* a, float* b);
//__device__ float gyration_radius(int				m_lig_begin,
//	int				m_lig_end,
//	const		atom_cl* atoms,
...
__device__ inline bool quaternion_is_normalized(float* q) {
	float q_pow = powf(q[0], 2) + powf(q[1], 2) + powf(q[2], 2) + powf(q[3], 2);
...
__device__ inline void quaternion_normalize_approx(float* q, float epsilon_fl) {
	const double s = powf(q[0], 2) + powf(q[1], 2) + powf(q[2], 2) + powf(q[3], 2);
	//if (isinf(s) != 0) {
	//	printf("\n Here!");
	//}
	// Omit one assert()
	if (fabsf(s - 1) < TOLERANCE)
		;
	else {
		const double a = sqrt(s);
		//if (a <= epsilon_fl) 
		//	printf("\nmutate: quaternion_normalize_approx ERROR! s = %0.16f",s); // Replace assert(a > epsilon_fl);
		for (int i = 0; i < 4; i++)q[i] *= (1 / a);
		//if (q[0] * q[1] * q[2] * q[3] == 0)
		//	printf("\n Here!");
		if (quaternion_is_normalized(q) != true) {
			q[0] = 1; q[1] = 0; q[2] = 0; q[3] = 0;// Replace assert(quaternion_is_normalized(q));
		}
	}
}
...
__device__ inline  float vec_distance_sqr(float* a, float* b) {
	return powf(a[0] - b[0], 2) + powf(a[1] - b[1], 2) + powf(a[2] - b[2], 2);
}
...
__device__ float gyration_radius(				int				m_lig_begin,
	...
	//return (counter > 0) ? acc / counter : 0;
}
